...
use Portfolio_db;
 SELECT * FROM cust_shopping LIMIT 5;
...
FROM cust_shopping
WHERE discount_applied = "Yes" AND  purchase_amount >= (SELECT AVG(purchase_amount) FROM cust_shopping);
...
-- total number of previou purchases and show the count of each segment.
WITH cust_type AS (
